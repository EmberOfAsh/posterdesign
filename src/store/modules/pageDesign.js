...
const getters = {
  ...
  viewImageDialog (state) {
    return state.viewImageDialog
  },
  editTextDialog (state) {
    return state.editTextDialog
  },
...
const actions = {
  ...
  loadPosterTemplate (store, data) {
    ...
    store.dispatch('updatePageSize', {width: width, height: height})
    let array = data.layouts
  ...
  updateViewImageDialog (store, data) {
    store.state.viewImageDialog.display = data.display
  },
  updateEditeTextDialog (store, data) {
    store.state.editTextDialog.display = data.display
  },
  /**
   * 保存操作历史，
   * 修改数据、移动完成后都会自动保存
