...
const effect = (render, type) => {
    
    let element = window.document.querySelector(render)
    
    let effectId = element.id + "Effect"
    //删除已有的特效
    let effectElement = window.document.getElementById(effectId)
    if(effectElement) {
        effectElement.parentNode.removeChild(effectElement)
    }
    //添加新特效
    effectElement = element.cloneNode(true)
    effectElement.id = effectId
    effectElement.classList.remove('hide')
    element.parentNode.insertBefore(effectElement, element.nextSibling)
    new FunkyLetters(effectElement, { colorize: true })
    let animator = new Animator(effectElement)
    animator.animate(type)
}
